...
const Mutations = {
  ...
  async requestReset(parent, args, ctx, info) {
    ...
    const mailRes = await transport.sendMail({
      from: '"Lani Huang" <laniywh@gmail.com>',
      to: user.email,
  ...
  async resetPassword(parent, args, ctx, info) {
    ...
    // 4. hash new password
    const password = await bcrypt.hash(args.password, 10);
    // 5. save new password and remove old resetToken
  ...
  async addToCart(parent, args, ctx, info) {
    ...
    // 2. query the users current cart item
    ...
    // 3. check if that item is already in the cart
    ...
    // 4. If not, create a cartItem
    const item = ctx.db.query.item({ where: { id: args.id } }, `{id title description price}`);
  ...
  async createOrder(parent, args, ctx, info) {
    ...
    const { userId } = ctx.request;
    if (!userId) throw new Error('You must signed in to complete this order.');
    ...
    const amount = user.cart.reduce((tally, cartItem) => tally + cartItem.item.price, 0);
    ...
    const order = await ctx.db.mutation.createOrder({
      data: {
        items: { create: orderItems }, // also crate orderItems here
        total: charge.amount,
        user: { connect: { id: userId } },
        charge: charge.id,
      },
    });
    // 6. clear user cart, delete cartItems
